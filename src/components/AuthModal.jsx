            ...
            >
              <div className="p-8">
                ...
                <div className="flex gap-1 p-1 bg-white/5 rounded-xl mb-6">
                  <button type="button" onClick={() => handleSwitchTab(true)}
                    className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all duration-300 ${isLogin ? 'bg-cyan-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}>
                    Log In
                  </button>
                  <button type="button" onClick={() => handleSwitchTab(false)}
                    className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all duration-300 ${!isLogin ? 'bg-cyan-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}>
                    Sign Up
                  </button>
                </div>
                ...
                <AnimatePresence mode="wait">
                  {error ? (
                    ...
                    <motion.p key="hint" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      ...
                      {isLogin ? 'Welcome back! Sign in to access your Watchlist.' : 'Create an account to save movies and sync progress.'}
                    </motion.p>
                  )}
                </AnimatePresence>
                    ...
                    {resetSent ? (
                      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                        ...
                        <button type="button" onClick={() => { setShowReset(false); setResetSent(false); setError(''); }}
                          className="mt-2 text-sm text-cyan-400 hover:text-cyan-300 font-semibold underline underline-offset-2 transition-colors">
                          ← Back to Log In
                        </button>
                      </motion.div>
                    ) : (
                      <>
                        <div className="relative">
                          ...
                          <input type="email" required value={resetEmail} onChange={(e) => setResetEmail(e.target.value)}
                            ...
                            className="w-full bg-black/20 border border-white/10 rounded-xl py-3 pl-11 pr-4 text-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 transition-all font-medium text-sm" />
                        </div>
                        <button type="submit" disabled={loading}
                          className="w-full py-3.5 flex justify-center items-center gap-2 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 text-white font-bold rounded-xl transition-all active:scale-[0.98]">
                    ...
                    {isLogin && (
                      <div className="flex justify-end -mt-1">
                        <button type="button" onClick={() => { setShowReset(true); setResetEmail(email); setError(''); }}
                          className="text-xs text-gray-500 hover:text-cyan-400 font-semibold underline underline-offset-2 transition-colors">
                          Forgot password?
                        </button>
                      </div>
                    )}
                    <button type="submit" disabled={loading}
                      className="w-full py-3.5 mt-2 flex justify-center items-center gap-2 disabled:opacity-50 text-white font-bold rounded-xl transition-all active:scale-[0.98] bg-cyan-600 hover:bg-cyan-500 shadow-[0_0_20px_rgba(6,182,212,0.3)]">
                      {loading && <FaSpinner className="animate-spin" />}
